...
def train(ctx: click.Context, train: TrainInput):
    ...
    def _train(experiment, optconfig, config, arch_path, metadata, pipeline_contents):
        ...
        #TODO: fix this
        extra_files, frozen_graph_path = _get_paths_for_extra_files(base_dir, model_dir)
        ...
        with ExitStack() as stack:
            ...
            #TODO: where is this folder actually being created?
            output_path = str(model_dir / 'validation')
            ...
            with tf.Session() as sess:
                with model.start_session():
                    for i, (bbox, centroid, z) in enumerate(truth_data):
...
def _get_paths_for_extra_files(artifact_path: Path, model_dir: Path):
    ...
    checkpoint_path = model_dir / ckpt_prefix
    pipeline_path = extras_path / 'pipeline.config'
    exported_dir = model_dir / 'frozen_model'
...
def _export_frozen_inference_graph(pipeline_config_path, checkpoint_path, output_directory):
    ...
    exporter.export_inference_graph(
        ...
        #TODO: ^^^^ changed write_inference_graph to TRUE; make sure to change back to FALSE!!!
