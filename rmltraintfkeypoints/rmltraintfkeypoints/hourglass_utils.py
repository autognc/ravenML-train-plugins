...
def create_hourglass_network(input_size, heatmap_size=64, num_classes=2, num_stacks=2, num_channels=128, bottleneck=None):
    ...
    front_features = create_front_module(input_layer, num_channels, bottleneck)
    ...
    out_layer = Concatenate(axis=1, name='merged_outputs')([
        Reshape((heatmap_size * heatmap_size * num_classes,), name='flatten_output_' + str(i))(out) 
        for i, out in enumerate(outputs)
    ])

    return Model(inputs=input_layer, outputs=out_layer, name='hourglass')
...
def create_front_module(input_layer, num_channels, bottleneck):
    # front module, input to 1/4 resolution
    # 1 7x7 conv + maxpooling
    # 3 residual block
    _x = Conv2D(64, kernel_size=(7, 7), strides=(2, 2), padding='same', activation='relu', name='front_conv_1x1_x1')(input_layer)
    _x = BatchNormalization()(_x)
